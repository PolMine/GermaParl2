...
target <- "GERMAPARL2"
corpus_version <- RcppCWB::corpus_property(corpus = target, property = "version")
```
...
colnames(tab_p) <- c("p-attribute", "description", "values")
  ...
  knitr::kable(tab_p, 
    ...
    kableExtra::footnote(number = "http://www.sfs.uni-tuebingen.de/Elwis/stts/stts.html") %>%
...
The boundaries of individual paragraphs and sentences are annotated. It is important to note that in table \@ref(tab:annotation-s-attributes) the value for the structural attributes `p` and `s` is NULL. This means here that regions for paragraphs and sentences are annotated and available within the corpus but they do not have any values which can be directly addressed. In practice this means that while context windows could be defined by sentence boundaries, a single sentence cannot be queried by name as of now. As shown above, paragraphs have an attribute "type" which is used to differentiate between paragraphs which contain ordinary speech and paragraphs which contain stage comments such as interjections.